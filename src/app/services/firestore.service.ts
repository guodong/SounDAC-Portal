...
export class FirestoreService {
  ...
  add<T>(ref: CollectionPredicate<T>, data, snackbarDisplay) {
    ...
    return this.col(ref).add({
      ...
      snackBar.open(snackbarDisplay + ' added with success.', 'Dismiss', {
        duration: 3000,
      });
      return true;
  ...
  update<T>(ref: DocPredicate<T>, data: any, snackbarDisplay?) {
    ...
    return this.doc(ref).update({
      ...
      if (snackbarDisplay) {
        snackBar.open(snackbarDisplay + ' updated with success.', 'Dismiss', {
          duration: 3000,
        });
      }
      return true;
    }).catch(function (error) {
      snackBar.open('An error occured while updating, please try again.');
      console.log(error);
      return false;
    });
  }
  ...
  delete<T>(ref: DocPredicate<T>, snackbarDisplay) {
    ...
    return this.doc(ref).delete().then(function () {
      snackBar.open(snackbarDisplay + ' deleted with success.', 'Dismiss', {
        duration: 3000,
      });
      return true;
    }).catch(function (error) {
